...
import { COMMENT_NODE, TEXT_NODE, ELEMENT_NODE } from './html-node-type';
...
function isCustomComponent(tagName: string, props: Object) {
  ...
  switch (tagName) {
    // Reserved SVG and MathML elements.
...
const DOMRenderer = ReactFiberReconciler({
    ...
    for (var propKey in props) {
      ...
      if (/on[A-Z][a-zA-Z]+/.test(propKey)) {
        ...
        if (typeof nextProp === 'string') {
          domElement.appendChild(createTextElement(nextProp, domElement));
  ...
  mutation: {
    ...
    commitUpdate(
      ...
      if (
        ...
        if (newProps.checked !== null) {
          setValueForProperty(domElement, 'checked', newProps);
        }
      }
    ...
    appendChild(container: Container, child: Instance | TextInstance): void {
      container.appendChild(child);
    },
  ...
  hydration: {
    ...
    canHydrateTextInstance(
      ...
      if (text === '' || instance.nodeType !== TEXT_NODE) {
        // Empty strings are not parsed by HTML
        return null;
      }
    ...
    hydrateInstance(): null | Array<mixed> {
      // instance: Instance,
      // type: string,
      // props: Props,
      // rootContainerInstance: Container,
...
const ReactDOM = {
  ...
  hydrate(
    ...
    warning('TODO: not a real hydrate method');
    let root = roots.get(container);
    if (!root) {
      root = DOMRenderer.createContainer(container);
      roots.set(container, root);
    ...
    DOMRenderer.updateContainer(element, root, null, callback);
    return DOMRenderer.getPublicRootInstance(root);
  }
};
